...
# lots of variables so lots of lines, but practical to 
# put all in init(data)
def init(data):
    ...
    # finds which game mode to bold; bolded one is the one we 
    # play
...
def mousePressed(event, data):
    ...
    * data.height and event.y < (boxLeft3 + boxLength) * 
    ...
    # let the user download their own stats
...
def keyPressed(event, data):
    ...
    # give points for current multiple choice answer
...
def timerFired(data):
    # timer up by 1 for game modes
    if ((data.page == 'Game' or data.page == 'Test') and data.pause == False):
        data.timerCalls += 1
    
    # term falls down in the fall mode
    data.cy += data.dy
...
def redrawAll(canvas, data):
    ...
    if (data.page == 'Stats'):
        ...
        # rest of code is to calculate stats
    ...
    if (data.page == 'Test'):
        # test questions buttons to display
    ...
    if (data.page == 'Cards'):
        ...
        canvas.create_text(data.width // 2, (.65) * data.height, 
                text = data.termDefs[data.currentTerm], font = "Helvetica %d" % data.fontSize, width = data.width // 2)
    ...
    # re-order the cards to optimize the order
    ...
    # game mode where definition is falling; user types in term
    if (data.page == 'Fall'):
    ...
    # normal flashcard game mode + stats
    if (data.page == 'Game'):
        specificCard = []
        for i in range(len(data.timeData)):
            if (data.timeData[i][0] == data.username and 
        ...
        # calculate stats for each card with respect to other users
        sorted_all = sorted(data.totalTimeData)
    ...
    # reached end of multiple choice test; show congrats screen
...
def run(width=300, height=300):
    ...
    def command1():
        if entry1.get() in data.usernames and entry2.get() in data.usernamePasswords: #Checks whether username and password are correct
            ...
            # save the data, but since new user, append
            # new entries for him
            file = open("information.txt", "w")
            print(data.usernames)
            file.write(str(data.usernames))
    ...
    # Buttons for the fall game
    top3 = Toplevel()
    entry3 = Entry(top) # word entry
    print("Hi")
    button3 = Button(top, text="Check Word", command=lambda:checkWord()) # checks word after user clicks button
    entry3.pack() 
    button3.pack()
    
    # check if the right word, and then send a new word
    def checkWord():
